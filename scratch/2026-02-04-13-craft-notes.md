...
**PR:** https://github.com/AustinEral/bosun/pull/24
**Branch:** polish/eventkind-name-method

**Session end:** 13:57 UTC
**Duration:** ~5 minutes

## Summary

Added `EventKind::name()` method:
1. Moved logic from standalone function to method on the type
2. Added doc comment with example
3. Added unit test verifying name() matches serde expectations
4. Doc test runs automatically via `cargo test`

This follows Rust idioms where type-specific behavior belongs on the type,
improving discoverability and encapsulation.
